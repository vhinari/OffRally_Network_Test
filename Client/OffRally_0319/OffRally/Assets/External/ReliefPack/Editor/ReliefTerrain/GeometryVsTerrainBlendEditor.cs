				...
				if (GUILayout.Button(new GUIContent("Stick to Blended Object along normals"))) {
					for(int i=0; i<targets.Length; i++) {
						...
						if (mft!=null) {						
							...
							_atarget.StickToBlendedObject();
							_atarget.GetComponent<Renderer>().castShadows=false;
							_atarget.SetupValues();
							EditorUtility.SetDirty(targets[i]);
						}						
					}
				...
				if (GUILayout.Button(new GUIContent("Stick to Blended Object downward"))) {
					for(int i=0; i<targets.Length; i++) {
						...
						if (mft!=null) {						
							RegisterUndoForMeshes(_atarget);
							if (mft.sharedMesh==_atarget.orig_mesh) _atarget.MakeMeshCopy();
							_atarget.StickToBlendedObject(false);
							_atarget.GetComponent<Renderer>().castShadows=false;
							EditorUtility.SetDirty(targets[i]);
						}
					}
				...
				if (_target.Sticked) {
					if (GUILayout.Button("Export OBJ")) {
						...
						if (path!="") {
							...
							if (_target.GetComponent(typeof(MeshFilter))) RTPObjExporter.MeshToFile((MeshFilter)_target.GetComponent(typeof(MeshFilter)), path);
						}										
					}
				}
