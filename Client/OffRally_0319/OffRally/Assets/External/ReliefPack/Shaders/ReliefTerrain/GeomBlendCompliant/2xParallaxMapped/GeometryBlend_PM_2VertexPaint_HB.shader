...
Shader "Relief Pack/GeometryBlend_2PM_VertexPaint_HB" {
	...
	void surf (Input IN, inout SurfaceOutput o) {
		...
		float4 global_color_value=tex2D(_ColorMapGlobal, IN.globalUV);
      	...
      	float2 tH=tex2D(_HeightMapCombined, IN.uv_MainTex).rg;
		#if !defined(RTP_SIMPLE_SHADING) 
	      	float2 uv=IN.uv_MainTex + ParallaxOffset(tH.x, _ExtrudeHeight, IN.viewDir.xyz)*(1-IN.color.a);
      		float2 uv2=IN.uv_MainTex + ParallaxOffset(tH.y, _ExtrudeHeight, IN.viewDir.xyz)*(1-IN.color.a);
      	#endif
