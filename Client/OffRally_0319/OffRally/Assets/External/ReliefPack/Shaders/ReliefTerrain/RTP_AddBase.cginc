...
inline half4 LightingCustomBlinnPhong_Deferred (in RTPSurfaceOutput s, out half4 outDiffuse, out half4 outSpecSmoothness, out half4 outNormal)
{
...
inline half4 LightingCustomBlinnPhong_DirLightmap (in RTPSurfaceOutput s, fixed4 color, fixed4 scale, half3 viewDir, bool surfFuncWritesNormal, out half3 specColor)
{
	#ifdef UNITY_PI
...
//	return;
	...
	IN.viewDir.z=saturate(IN.viewDir.z); // czasem wystepuja problemy na krawedziach widocznosci (viewDir.z nie powinien byc jednak ujemny)
		...
		fixed4 hMapVal_YZ = tex2D(_TERRAIN_HeightMap3, _INPUT_uv.yz, _ddxMain.yz, _ddyMain.yz);
		fixed4 hMapVal_XY = tex2D(_TERRAIN_HeightMap3, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy);
		fixed4 hMapVal_XZ = tex2D(_TERRAIN_HeightMap3, _INPUT_uv.xz, _ddxMain.xz, _ddyMain.xz);
	...
	#else
		#ifdef USE_EXTRUDE_REDUCTION
			tHA=saturate(lerp(tex2D(_TERRAIN_HeightMap3, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy), 1, PER_LAYER_HEIGHT_MODIFIER89AB)+0.001);
		#else
			tHA=saturate(tex2D(_TERRAIN_HeightMap3, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy)+0.001);
		#endif	
	#endif
	splat_control1 *= tHA;
...
//#endif		 			
			...
			float4 gloss=0;
				...
				if (triplanar_blend_flag) {
					...
					if (triplanar_blend.x>0.02) {
						...
						#else
							c = tex2D(_SplatC0, uvTRI1.xy, _ddxMain.yz, _ddyMain.yz); col += _MixBlendtmp.x * c.rgb; tmp_gloss.r=c.a;
							c = tex2D(_SplatC1, uvTRI1.xy, _ddxMain.yz, _ddyMain.yz); col += _MixBlendtmp.y * c.rgb; tmp_gloss.g=c.a;
							c = tex2D(_SplatC2, uvTRI1.xy, _ddxMain.yz, _ddyMain.yz); col += _MixBlendtmp.z * c.rgb; tmp_gloss.b=c.a;
							c = tex2D(_SplatC3, uvTRI1.xy, _ddxMain.yz, _ddyMain.yz); col += _MixBlendtmp.w * c.rgb; tmp_gloss.a=c.a;
						#endif				
					...
					if (triplanar_blend.y>0.02) {
						...
						dir.y=triplanar_flip.y ? -dir.y:dir.y;						
							...
							c = tex2Dlod(_SplatAtlasC, float4(uvSplat23.zw, _MipActual.xx)); col += _MixBlendtmp.w * c.rgb; tmp_gloss.a = c.a;
						#else
							c = tex2D(_SplatC0, uvTRI2.xy, _ddxMain.xy, _ddyMain.xy); col += _MixBlendtmp.x * c.rgb; tmp_gloss.r=c.a;
							c = tex2D(_SplatC1, uvTRI2.xy, _ddxMain.xy, _ddyMain.xy); col += _MixBlendtmp.y * c.rgb; tmp_gloss.g=c.a;
							c = tex2D(_SplatC2, uvTRI2.xy, _ddxMain.xy, _ddyMain.xy); col += _MixBlendtmp.z * c.rgb; tmp_gloss.b=c.a;
							c = tex2D(_SplatC3, uvTRI2.xy, _ddxMain.xy, _ddyMain.xy); col += _MixBlendtmp.w * c.rgb; tmp_gloss.a=c.a;
						#endif				
					...
					if (triplanar_blend.z>0.02) {
						...
						dir.y=triplanar_flip.z ? dir.y:-dir.y;
							...
							c = tex2Dlod(_SplatAtlasC, float4(uvSplat23.zw, _MipActual.xx)); col += _MixBlendtmp.w * c.rgb; tmp_gloss.a = c.a;
						#else
							c = tex2D(_SplatC0, uvTRI3.xy, _ddxMain.xz, _ddyMain.xz); col += _MixBlendtmp.x * c.rgb; tmp_gloss.r=c.a;
							c = tex2D(_SplatC1, uvTRI3.xy, _ddxMain.xz, _ddyMain.xz); col += _MixBlendtmp.y * c.rgb; tmp_gloss.g=c.a;
							c = tex2D(_SplatC2, uvTRI3.xy, _ddxMain.xz, _ddyMain.xz); col += _MixBlendtmp.z * c.rgb; tmp_gloss.b=c.a;
							c = tex2D(_SplatC3, uvTRI3.xy, _ddxMain.xz, _ddyMain.xz); col += _MixBlendtmp.w * c.rgb; tmp_gloss.a=c.a;
						#endif	
					...
					#else
						c = tex2D(_SplatC0, uvTRI.xy, _ddxMainTRI.xy, _ddyMainTRI.xy); col += c.rgb*splat_control1.x; tmp_gloss.r=c.a;
						c = tex2D(_SplatC1, uvTRI.xy, _ddxMainTRI.xy, _ddyMainTRI.xy); col += c.rgb*splat_control1.y; tmp_gloss.g=c.a;
						c = tex2D(_SplatC2, uvTRI.xy, _ddxMainTRI.xy, _ddyMainTRI.xy); col += c.rgb*splat_control1.z; tmp_gloss.b=c.a;
						c = tex2D(_SplatC3, uvTRI.xy, _ddxMainTRI.xy, _ddyMainTRI.xy); col += c.rgb*splat_control1.w; tmp_gloss.a=c.a;
					#endif						
					gloss=tmp_gloss;
				...
				//
					...
					c = tex2D(_SplatC0, rayPos.xy, _ddxMain.xy, _ddyMain.xy); col = splat_control1.x * c.rgb; gloss.r = c.a;
					c = tex2D(_SplatC1, rayPos.xy, _ddxMain.xy, _ddyMain.xy); col += splat_control1.y * c.rgb; gloss.g = c.a;
					c = tex2D(_SplatC2, rayPos.xy, _ddxMain.xy, _ddyMain.xy); col += splat_control1.z * c.rgb; gloss.b = c.a;
					c = tex2D(_SplatC3, rayPos.xy, _ddxMain.xy, _ddyMain.xy); col += splat_control1.w * c.rgb; gloss.a = c.a;
				#endif
			...
			col*=dot(splat_control1, _LayerBrightness89AB);  
				...
				rayPos.w=mip_selector.x+rtp_mipoffset_bump;
					...
					rayPos.w += snow_depth;
				#endif				
				normals_combined = tex2D(_BumpMap89, rayPos.xy, _ddxMain.xy, _ddyMain.xy).rgba*splat_control1.rrgg;
				normals_combined+=tex2D(_BumpMapAB, rayPos.xy, _ddxMain.xy, _ddyMain.xy).rgba*splat_control1.bbaa;
				n.xy=(normals_combined.rg+normals_combined.ba)*2-1;
				n.xy*=_uv_Relief_z;
				n.z = sqrt(1 - saturate(dot(n.xy, n.xy)));
				o.Normal=n;
...
//#endif
			// snow color
				...
				// ponownie przelicz uvSplat01 i uvSplat23 dla triplanar (nadpisane powyżej)
					...
					#if defined(RTP_PM_SHADING)
						#ifdef USE_EXTRUDE_REDUCTION
				 			hgtXY = triplanar_blend.y * dot(splat_control1, lerp(tex2D(_TERRAIN_HeightMap3, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy), 1, PER_LAYER_HEIGHT_MODIFIER89AB));
						#else
				 			hgtXY = triplanar_blend.y * dot(splat_control1, tex2D(_TERRAIN_HeightMap3, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy));
						#endif						
		 	...
		 	//
				...
				for(int i=0; i<_TERRAIN_SHADOW_STEPS; i++) {
					...
					if (hit_flag) {
						...
						float scl=dh_prev / ((_h-h_prev) - EyeDirTan.z);
						rayPos.xyz-=EyeDirTan*(1 - scl); // back
						EyeDirTan=IN.color.xyz*_TERRAIN_HeightMap3_TexelSize.x*exp2(rayPos.w)*_TERRAIN_WAVELENGTH_SHADOWS;
						EyeDirTan.z/=max(0.001, _TERRAIN_ExtrudeHeight*COLOR_DAMP_VAL);
						float smooth_val=0;
						float break_val=_TERRAIN_ExtrudeHeight*_TERRAIN_ShadowSmoothing;
						for(int i=0; i<_TERRAIN_SHADOW_SMOOTH_STEPS; i++) {
...
//#endif		
			...
			float4 gloss=0;			
				...
				float4 tmp_gloss;
					...
					c = tex2D(_SplatC0, uvTRI.xy, _ddxMainTRI.xy*mipTweak.x, _ddyMainTRI.xy*mipTweak.x); col += c.rgb*splat_control1.x; tmp_gloss.r=c.a;
					c = tex2D(_SplatC1, uvTRI.xy, _ddxMainTRI.xy*mipTweak.y, _ddyMainTRI.xy*mipTweak.y); col += c.rgb*splat_control1.y; tmp_gloss.g=c.a;
					c = tex2D(_SplatC2, uvTRI.xy, _ddxMainTRI.xy*mipTweak.z, _ddyMainTRI.xy*mipTweak.z); col += c.rgb*splat_control1.z; tmp_gloss.b=c.a;
					c = tex2D(_SplatC3, uvTRI.xy, _ddxMainTRI.xy*mipTweak.w, _ddyMainTRI.xy*mipTweak.w); col += c.rgb*splat_control1.w; tmp_gloss.a=c.a;
				#endif						
				gloss=tmp_gloss;				
				...
				//
					...
					float4 mipTweak=exp2(MIPmult89AB);
					c = tex2D(_SplatC0, _INPUT_uv.xy, _ddxMain.xy*mipTweak.x, _ddyMain.xy*mipTweak.x); col = splat_control1.x * c.rgb; gloss.r = c.a;
					c = tex2D(_SplatC1, _INPUT_uv.xy, _ddxMain.xy*mipTweak.y, _ddyMain.xy*mipTweak.y); col += splat_control1.y * c.rgb; gloss.g = c.a;
					c = tex2D(_SplatC2, _INPUT_uv.xy, _ddxMain.xy*mipTweak.z, _ddyMain.xy*mipTweak.z); col += splat_control1.z * c.rgb; gloss.b = c.a;
					c = tex2D(_SplatC3, _INPUT_uv.xy, _ddxMain.xy*mipTweak.w, _ddyMain.xy*mipTweak.w); col += splat_control1.w * c.rgb; gloss.a = c.a;				
				#endif
...
//			float RTP_IBL_SpecStrength=dot(splat_control1, RTP_IBL_SpecStrength89AB)+dot(splat_control2, RTP_IBL_SpecStrength4567);
	...
	#ifdef RTP_CAUSTICS
	{
		if (damp_fct_caustics>0 && _uv_Relief_w<1) {
			...
			tHB=saturate(tex2D(_TERRAIN_HeightMap, _INPUT_uv.xy, _ddxMain.xy, _ddyMain.xy)+0.001); // 1 (0-3)
