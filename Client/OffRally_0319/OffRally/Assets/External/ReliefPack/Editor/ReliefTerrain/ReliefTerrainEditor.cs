...
public class ReliefTerrainEditor : Editor {
	...
	void OnEnable() {
		...
		if (_target.globalSettingsHolder!=null && !_target.globalSettingsHolder.dont_check_for_interfering_terrain_replacement_shaders) {
						...
						EditorGUILayout.BeginHorizontal();
							...
							EditorGUI.BeginDisabledGroup(!RTP_TESSELLATION_SAMPLE_TEXTURE);
								if (checkChange(ref _targetRT.NormalGlobal, (Texture2D)EditorGUILayout.ObjectField(_targetRT.NormalGlobal, typeof(Texture2D), false, GUILayout.MinHeight(100), GUILayout.MinWidth(100), GUILayout.MaxWidth(100)))) {
									...
									if (_importer) {
										TextureImporter tex_importer=(TextureImporter)_importer;
										if (tex_importer.textureFormat!=TextureImporterFormat.RGBA32 || tex_importer.textureFormat!=TextureImporterFormat.ARGB32) {
							...
							EditorGUILayout.BeginVertical();
								...
								EditorGUI.BeginDisabledGroup(!RTP_TESSELLATION_SAMPLE_TEXTURE);
									EditorGUILayout.BeginHorizontal();
									EditorGUILayout.LabelField("Normal strength", GUILayout.MinWidth(130), GUILayout.MaxWidth(130));
									checkChange(ref _target.global_normalMap_multiplier, EditorGUILayout.Slider(_target.global_normalMap_multiplier, 0f, 6f));
									EditorGUILayout.EndHorizontal();
								EditorGUI.EndDisabledGroup();
								EditorGUILayout.BeginHorizontal();
								EditorGUILayout.LabelField("Tess. substeps (close)", GUILayout.MinWidth(130), GUILayout.MaxWidth(130));
									if (checkChange(ref _target.trees_pixel_blend_val, EditorGUILayout.Slider(_target.trees_pixel_blend_val, 1f, 63f))) {
									_target.trees_pixel_blend_val=Mathf.Floor((_target.trees_pixel_blend_val-1)/2)*2+1;
								}
								EditorGUILayout.EndHorizontal();
								EditorGUILayout.BeginHorizontal();
								EditorGUILayout.LabelField("Tess. substeps (far)", GUILayout.MinWidth(130), GUILayout.MaxWidth(130));
								if (checkChange(ref _target.global_normalMap_farUsage, EditorGUILayout.Slider(_target.global_normalMap_farUsage, 1f, 63f))) {
									_target.global_normalMap_farUsage=Mathf.Floor((_target.global_normalMap_farUsage-1)/2)*2+1;
								}
								EditorGUILayout.EndHorizontal();
								EditorGUI.BeginDisabledGroup(RTP_TESSELLATION_SAMPLE_TEXTURE);
									if (RTP_TESSELLATION_SAMPLE_TEXTURE) _target._Phong=0;
									EditorGUILayout.BeginHorizontal();
									EditorGUILayout.LabelField("Phong smoothening", GUILayout.MinWidth(130), GUILayout.MaxWidth(130));
								...
								if (terrain!=null) {
									EditorGUILayout.BeginHorizontal();
									EditorGUILayout.LabelField("Terrain pixel error", GUILayout.MinWidth(130), GUILayout.MaxWidth(130));
									terrain.heightmapPixelError = EditorGUILayout.Slider(terrain.heightmapPixelError, 1f, 200f);
									EditorGUILayout.EndHorizontal();
									...
									if (GUILayout.Button("Reapply height texture to terrain data")) {
										if (EditorUtility.DisplayDialog("Warning", "Are you sure to overwrite terrain height data ?","Yes","Cancel")) {
											...
											}
											{
												...
												int sampleSize = _targetRT.NormalGlobal.width/terrainData.heightmapResolution;
													...
													for(int _z = 0; _z<terrainData.heightmapResolution; _z++) {
														...
														for(int _xS = 0; _xS<sampleSize; _xS++) {
															for(int _zS = 0; _zS<sampleSize; _zS++) {
																Color col = _targetRT.NormalGlobal.GetPixel(_xB+_xS, _zB+_zS);
																if (gVal<col.g) gVal=col.g;
																if (rVal<col.r) rVal=col.r;
															}
														}
														heights[_z,_x] = ((1.0f/255)*gVal + rVal);
													}
												}
									...
									EditorGUILayout.BeginHorizontal();
									EditorGUILayout.LabelField("Displacement height", GUILayout.MinWidth(130), GUILayout.MaxWidth(130));
									if (checkChange(ref _target.tessHeight, EditorGUILayout.FloatField(_target.tessHeight))) {
										...
										if (mf && mf.sharedMesh) {
											...
											center.y=_target.tessHeight/_targetRT.transform.localScale.y*0.5f;
